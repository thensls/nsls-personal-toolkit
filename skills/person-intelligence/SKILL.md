...
**Signal — Quick Notes** (when `SIGNAL_INGEST=1` AND the person is `signal_eligible` — any tracked person with NSLS email, not board/external; see `list_relationships.py`):
...
**Scope: any tracked person who is `signal_eligible` (NSLS email, not board/external).** **Raw Quick Notes never enter the vault** — `fetch_signal.py`
...
| **Signal** | Quick Notes wins/friction/sentiment/goal-health (any tracked person who is `signal_eligible` — NSLS email, not board/external; see `list_relationships.py`), `SIGNAL_INGEST=1`. Distilled into `## Signal Read` + advisory `## How to Support`. | `signal_*` MCP (Phase 1) |
