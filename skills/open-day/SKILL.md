...
- If Step 2k was skipped entirely (no MCP, no data), keep the `status:` key but omit the health keys AND drop the "Yesterday:" line from Morning Check-in.
- **If the daily note already exists** (close-day seeded it last night) and has existing frontmatter, merge: update the health keys, preserve everything else (including `status`). If it has no frontmatter, prepend the block.
