...
2. **Meeting rows.** For each meeting from Step 1a's `## Meetings` data, joined to that meeting's entry in Step 1a's `meeting_topics`, resolve it through the cascade: an attendee match in `~/.claude/portfolio-role-map.txt` (role) → the meeting's `topic_sections` from Step 1a's Fathom fetch, mapped to quadrants per `portfolio-attribution.md` §3 and apportioned by the span between consecutive timestamps (topic — may split across quadrants) → the meeting's mapped project's `portfolio-category` (project) → `unresolved`. Record which rule fired as `resolved_by` for every row. A meeting whose `topic_sections` came back empty skips the topic rung — note the reason at the confirm gate rather than letting it read as a judgment call. **Two role-rung behaviours must also be reported, not swallowed.** A rule may carry an `<=N` attendee cap (`<=1` is a 1:1, because Step 1a passes Fathom `calendar_invitees`, which excludes the builder); a capped rule that did not fire simply falls through, which is correct and needs no note. But when two matching rules **disagree**, the rung declines rather than resolving by attendee order, and the `Resolution.note` names the conflicting quadrants — print that note at the confirm gate, because a meeting that fell to topic *after* a role conflict is a role map that needs fixing, not a clean topic read. And when the role map has **no active rules at all** (absent file, or every rule commented out), say so once: `role map has no active rules — the role rung cannot fire this week`. That is a reportable state, not zero matches found.
