...
**Branch drift to resolve before Phase 3 (logged 2026-06-21):** `pp-cli-visual` (installed) has the default-ON visual companion + graceful chat fallback in open-day/close-day; our `pp-cowork-companion` branch is *behind* on that (still says "default off"). The fallback clause ("on surfaces that can't run the companion, finish in chat") is the exact seam the cowork artifact branch plugs into — so rebase/merge that flip in before adding the Desktop branch, or the two will conflict.
