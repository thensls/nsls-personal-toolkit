...
- **O2 — Artifact save round-trip mechanics. ✅ RESOLVED (2026-06-21, real cowork session).** The channel is **`sendPrompt(text)`** — a global function the artifact runtime exposes: "sends a message to chat as if the user typed it." On Save, the artifact calls `sendPrompt("SAVE_COUNTER " + JSON.stringify(state))`; it lands in the thread **as a visible user message**, Claude parses it and writes the vault in one turn. Empirically confirmed: counter value reached Claude and `50-reference/cowork-counter.md` was written. **NOT available:** `window.claude.complete()` (does not exist in cowork — it's from the older standalone-artifact runtime); `postMessage` to parent (posts but dies in the host frame, never enters model context — false positive). **Design consequence:** the hand-back is a real chat turn (visible user message + Claude write turn), not a silent callback — so batched/explicit saves aren't just preferred, they're *required*; per-tap autosave would flood the thread and burn tokens. The save trigger emits the artifact's full serialized state in one `sendPrompt`. Also: the artifact cannot read its own rendered DOM back to Claude except by `sendPrompt`-ing it — so any "report runtime state" must be an explicit button.
